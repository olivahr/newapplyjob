...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function buildTimeline(){
    ...
    for (let i = 1; i <= TOTAL; i++) {
      ...
      item.setAttribute("data-go", String(i));
  ...
  buildTimeline();
  let mapItems = Array.from(document.querySelectorAll(".mstep"));
  ...
  function isStepValid(){
    ...
    // Radios por grupo (Safari-safe sin CSS.escape)
    const radios = required.filter(el => el.type === "radio");
    if (radios.length) {
      const names = Array.from(new Set(radios.map(r => r.name).filter(Boolean)));

      for (const name of names) {
        const group = Array.from(active.querySelectorAll(`input[type="radio"][name="${name}"]`));
  ...
  document.addEventListener("click", (e) => {
    ...
    if (map) {
      const go = Number(map.getAttribute("data-go") || "0");
      if (go && go <= current) setActive(go);
    }
  });
